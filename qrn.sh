#!/bin/sh
# qrn: Read the Word of God from your terminal
...
show_help() {
	...
	echo
	echo "  -l      list Quran translation books"
	echo "  -W      no line wrap"
	echo "  -h      show help"
	echo
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data qrn.tsv | awk -v cmd=list "$(get_data qrn.awk)"
		exit
	elif [ "$1" = "-W" ]; then
		export QRN_NOLINEWRAP=1
		shift
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data qrn.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data qrn.awk)" | ${PAGER}
	done
	exit 0
fi

get_data qrn.tsv | awk -v cmd=ref -v ref="$*" "$(get_data qrn.awk)" | ${PAGER}
