BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["QRN_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["QRN_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["QRN_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["QRN_NOLINEWRAP"] != "" && ENVIRON["QRN_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
